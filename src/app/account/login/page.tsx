...
export default function LoginPage() {
  ...
  // ── Post-login Phone Capture (for Google/Email)
  ...
  useEffect(() => {
    // If user is already logged in but missing a phone number, show capture screen
  ...
  async function checkUserPhone(uid: string) {
    try {
      ...
      if (snap.exists() && snap.data().phone) {
        // All good, redirect home or admin if they were trying to go there
        // (Handled by the UI buttons)
      } else {
  ...
  async function handleVerifyDbConnection() {
    ...
    try {
      ...
      toast({ 
        ...
        description: "Successfully wrote test data to Firestore users collection." 
      });
    } catch (e: any) {
      toast({ 
        variant: "destructive", 
  ...
  async function handleSendOtp() {
    ...
    try {
      ...
      const hint = getTestOtp(fullPhone);
      setTestOtpHint(hint);
      setPhoneStep('otp');
      startResendTimer();
      ...
      if (error.code === 'auth/admin-restricted-operation' || error.message.includes('billing')) {
        toast({ 
          ...
          description: 'SMS quota reached or billing disabled. Try Google Login or use Test Mode.' 
        });
      } else {
        toast({ variant: 'destructive', title: 'Error', description: error.message });
      }
  ...
  async function handleVerifyOtp() {
    ...
    try {
      ...
      if (!cred.user.displayName) setPhoneStep('profile');
      else router.push('/');
    } catch (error: any) {
      ...
      <div className="flex min-h-[80vh] items-center justify-center py-12 px-4">
        <div className="w-full max-w-md bg-white p-8 md:p-12 rounded-3xl shadow-2xl border border-slate-100 space-y-8 text-center">
          ...
          <form onSubmit={handlePhoneCaptureSubmit} className="space-y-6">
            ...
            <Button
              type="submit"
              disabled={isCapturingPhone || signupPhone.length !== 10}
      ...
      <div className="w-full max-w-md bg-white p-8 md:p-12 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-slate-100 space-y-8">
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <p className="text-muted-foreground text-sm">
            Access your curated luxury fashion community.
          </p>
        </div>
        ...
        {unauthorizedDomain && (
          <Alert variant="destructive" className="rounded-2xl border-red-100 bg-red-50">
            ...
            <AlertDescription className="space-y-3 pt-2">
              ...
              <div className="bg-white p-2 rounded-lg border border-red-100 font-mono text-[10px] break-all select-all">
                {unauthorizedDomain}
              </div>
              <p className="text-[10px] italic">Paste this into <strong>Authentication &gt; Settings &gt; Authorized Domains</strong></p>
            </AlertDescription>
          </Alert>
        )}
        ...
        {user ? (
          <div className="space-y-6">
            <div className="p-6 rounded-2xl bg-secondary/20 border border-primary/10 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
                  {user.isAnonymous ? 'Guest Access' : 'Verified Profile'}
                </p>
              </div>
              {user.isAnonymous && <Zap className="h-4 w-4 text-accent animate-pulse" />}
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <Button 
                onClick={handleVerifyDbConnection} 
                disabled={isVerifyingDb}
                variant="outline" 
                className="w-full h-12 rounded-xl border-slate-200 font-bold uppercase text-[9px] tracking-widest gap-2"
              >
                {isVerifyingDb ? <Loader2 className="h-3 w-3 animate-spin" /> : <ShieldCheck className="h-3 w-3 text-green-500" />}
              ...
              <Button 
                ...
                className="text-muted-foreground hover:text-red-500 text-[10px] font-bold uppercase tracking-widest"
              >
                Sign Out
              </Button>
            </div>
          ...
          <div className="space-y-6">
            ...
            <Button
              ...
              className="w-full h-14 rounded-xl bg-white border-2 border-slate-100 hover:bg-slate-50 text-slate-900 font-bold text-sm gap-3 shadow-sm transition-all active:scale-95"
            >
              {isGoogleLoading ? (
            ...
            <div className="space-y-4">
              {phoneStep === 'input' && (
                <div className="space-y-4">
                  ...
                  >
                    {isSendingOtp ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Send Test OTP'}
                  </Button>
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <Input
                      ...
                      className="h-14 rounded-xl border-slate-200 text-center tracking-[0.5em] font-bold text-xl"
                    />
                    {testOtpHint && (
                      <p className="text-[10px] text-amber-600 font-bold text-center pt-1">
                        Dev Hint: Your OTP is {testOtpHint}
                      </p>
                    )}
                  </div>
                  <Button
                    ...
                    className="w-full h-12 rounded-xl bg-slate-900 hover:bg-black text-white font-bold text-xs uppercase tracking-widest"
                  >
            ...
            <div className="pt-4 border-t border-slate-50 space-y-4">
              <p className="text-[10px] text-center text-slate-400 uppercase tracking-widest">Developer Utilities</p>
              <Button 
                onClick={handleAdminBypass} 
                variant="outline" 
                className="w-full h-12 rounded-xl border-dashed border-primary/20 text-primary hover:bg-primary/5 font-bold uppercase text-[10px] tracking-widest gap-2"
              >
                <Zap className="h-3 w-3" /> Skip OTP & Enter Admin Panel
              </Button>
            </div>
          </div>
        )}
