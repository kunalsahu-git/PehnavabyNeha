...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="flex flex-col w-full pb-20">
      {/* Breadcrumb - Hidden on very small screens or made compact */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16 max-w-6xl mx-auto">
          ...
          <div className="space-y-4">
            <div className="flex flex-col-reverse md:flex-row gap-4">
              {/* Thumbnails - Horizontal on mobile, Vertical on tablet+ */}
              <div className="flex md:flex-col gap-3 md:w-20 overflow-x-auto md:overflow-y-auto no-scrollbar pb-2 md:pb-0">
                {images.map((img, idx) => (
                  <button
                    ...
                    className={cn(
                      "relative flex-shrink-0 w-16 md:w-20 aspect-square rounded-md overflow-hidden border-2 transition-all",
              ...
              <div className="flex items-center gap-3 md:gap-4">
                ...
                {product.originalPrice && (
                  <>
                    ...
                    <span className="text-destructive font-bold text-[10px] md:text-xs bg-destructive/10 px-2.5 py-0.5 rounded-full uppercase tracking-tighter">
                      Save {discount}%
                    </span>
                  </>
                )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.sizes.map(size => (
                      <button
                        ...
                        className={cn(
                          "min-w-[2.5rem] md:min-w-[3rem] h-10 md:h-12 px-3 md:px-4 flex items-center justify-center rounded-md border text-[10px] md:text-xs font-bold transition-all uppercase",
                          selectedSize === size 
                  ...
                  <div className="flex gap-2.5 md:gap-3">
                    {product.colors.map(color => (
                      <button
                        ...
                        className={cn(
                          "w-8 h-8 md:w-10 md:h-10 rounded-full border-2 p-1 transition-all",
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                <Button className="w-full h-14 md:h-16 bg-slate-900 text-white hover:bg-black font-bold rounded-full flex items-center justify-center gap-2 md:gap-3 text-xs md:text-sm tracking-[0.1em] shadow-xl active:scale-95 transition-transform">
                  ...
                  <div className="flex items-center gap-1 opacity-80 border-l border-white/20 pl-2 md:pl-3">
                    <Image src="https://placehold.co/40x24/png?text=UPI" width={32} height={20} alt="UPI" className="invert brightness-0" />
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row gap-10 md:gap-12 items-start justify-between">
              ...
              <div className="w-full space-y-2 md:space-y-3 mt-4">
                {[5, 4, 3, 2, 1].map((star) => (
                  <div key={star} className="flex items-center gap-3">
                    ...
                    <span className="text-[9px] md:text-[10px] font-bold text-muted-foreground w-4">
                      {star === 5 ? 1 : star === 4 ? 2 : 0}
                    </span>
                  </div>
                ))}
            ...
            <div className="w-full md:w-auto mt-6 md:mt-0">
               <Button className="w-full md:w-auto bg-slate-800 text-white rounded-md px-10 h-11 md:h-12 hover:bg-slate-900 transition-colors text-xs font-bold">
                 Write a review
               </Button>
            </div>
          </div>

          {/* Review Cards Grid - Scrollable on small screens, Grid on desktop */}
            ...
            {REVIEWS.map((review) => (
              <div key={review.id} className="bg-white p-6 md:p-8 rounded-sm shadow-sm border border-slate-100 flex flex-col gap-3 md:gap-4 min-w-[280px] md:min-w-0">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground leading-relaxed mt-2 italic">
                    "{review.comment}"
                  </p>
                </div>
              </div>
      ...
      {relatedProducts.length > 0 && (
        <section className="container mx-auto px-4 mt-8 md:mt-10 border-t pt-16 md:pt-20">
          <div className="flex flex-col items-center text-center mb-10 md:text-center md:mb-16 space-y-3 md:space-y-4">
