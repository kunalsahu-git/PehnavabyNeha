...
import { CATEGORIES, CategoryMeta } from "@/lib/store-data";
...
export default function CollectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const category: CategoryMeta | undefined = CATEGORIES.find(c => c.slug === slug);

  // Firestore query — switches based on slug
  const productsQuery = useMemoFirebase(() => {
    if (slug === 'new-arrivals') return getNewArrivalsQuery(db);
    if (slug === 'sale') return getSaleProductsQuery(db);
    return getProductsByCategoryQuery(db, slug);
  }, [db, slug]);

  const { data: rawProducts, isLoading } = useCollection<ProductData>(productsQuery);

  // Client-side filter + sort
  const filteredAndSortedProducts = useMemo(() => {
    let products = (rawProducts ?? []) as WithId<ProductData>[];
  ...
  return (
    <div className="flex flex-col w-full pb-20">
      ...
      <section className="relative h-[30vh] md:h-[35vh] w-full flex items-center justify-center overflow-hidden">
        {bannerImage ? (
          <Image src={bannerImage} alt={categoryName} fill className="object-cover brightness-50" priority data-ai-hint={category?.bannerHint} />
        ) : (
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-3xl space-y-4">
          ...
          <h1 className="text-4xl md:text-5xl font-headline font-bold leading-tight uppercase tracking-wider">{categoryName}</h1>
          {category?.description && (
            <p className="text-sm md:text-base font-light max-w-xl mx-auto opacity-90 line-clamp-2">{category.description}</p>
          )}
        </div>
      </section>
                  ...
                  <div className="py-8 space-y-10">
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold text-primary px-2 py-1 bg-primary/5 rounded">₹{priceRange[0]} – ₹{priceRange[1]}</span>
                      </div>
                      <div className="px-2">
                        <Slider defaultValue={[0, 15000]} max={15000} step={500} value={priceRange} onValueChange={setPriceRange} className="py-4" />
                        <div className="flex items-center justify-between text-xs font-medium text-muted-foreground mt-2">
                          <span>₹0</span><span>₹15000+</span>
                        </div>
                      </div>
                    </div>
                    {/* Colors */}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {AVAILABLE_FABRICS.map(fabric => (
                          <label key={fabric} className="flex items-center gap-3 text-sm cursor-pointer"
        ...
        <section className="container mx-auto px-4 py-20 border-t border-b bg-secondary/10 mt-20 rounded-3xl">
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-col items-center text-center mb-12 space-y-4">
              <h2 className="text-4xl md:text-5xl font-headline font-bold uppercase tracking-wider">{categoryName}</h2>
